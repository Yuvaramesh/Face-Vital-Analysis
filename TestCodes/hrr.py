...
6. Measurements start calculating after 10 seconds of data
    ...
    def setup_individual_metrics(self, parent):
        ...
        metrics_data = [
            ...
            ("Systolic BP", "blood_pressure_sys", "mmHg", self.bp_sys_values, "darkred"),
            ("Diastolic BP", "blood_pressure_dia", "mmHg", self.bp_dia_values, "maroon")
        ]
        ...
        for name, key, unit, data_deque, color in metrics_data:
            ...
            if key in ["blood_pressure_sys", "blood_pressure_dia"]:
                # For blood pressure, show combined format
                if key == "blood_pressure_sys":
                    self.result_labels["blood_pressure"] = ttk.Label(
                        number_frame, text="--/-- mmHg", 
    ...
    def setup_signals_tab(self, parent):
        ...
        self.metrics_anim = FuncAnimation(plt.figure(), self.update_metrics_plots, interval=200, blit=False)
    ...
    def update_metrics_plots(self, frame):
        ...
        for key, data_deque in metrics_data:
            if key in self.metric_lines and len(data_deque) > 1:
                ...
                self.metric_lines[key].set_data(time_axis, list(data_deque))
                self.metric_axes[key].relim()
                self.metric_axes[key].autoscale_view()
                self.metric_figures[key].canvas.draw_idle()
        ...
        if len(self.bp_sys_values) > 1 and len(self.bp_dia_values) > 1:
            ...
            self.metric_lines["blood_pressure_sys"].set_data(time_axis_bp, list(self.bp_sys_values))
            self.metric_lines["blood_pressure_dia"].set_data(time_axis_bp, list(self.bp_dia_values))
            self.metric_axes["blood_pressure"].relim()
            self.metric_axes["blood_pressure"].autoscale_view()
            self.metric_figures["blood_pressure"].canvas.draw_idle()
    ...
    def stop_monitoring(self):
        ...
        messagebox.showinfo("Completed", "30-second monitoring completed! You can now export your data or start a new session.")
    ...
    def export_to_excel(self, filename):
        ...
        try:
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                # Summary sheet
                summary_data = {
                    'Metric': ['Session Start', 'Session End', 'Duration (seconds)', 'Total Frames', 'Total Measurements'],
                    'Value': [
    ...
    def generate_health_report(self):
        ...
        report = f"""
30-SECOND HEALTH MONITORING REPORT
=================================
...
- Duration: 30 seconds
- Total Frames Captured: {len(self.session_data['raw_ppg_data'])}
- Total Measurements: {len(self.session_data['measurements'])}
...
{self.get_recommendations()}
    ...
    def get_recommendations(self):
        ...
        recommendations.append("• Consider regular health monitoring sessions")
